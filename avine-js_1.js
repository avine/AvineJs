...
(function (window) {
	...
	Core.prototype = {
		...
		// Clone the Core instance to create parallel call stacks
		clone: function (callback, listeners) {
			var Clone = function () {
				this._stack = Core._emptyStack(); // Each clone has his own stack
				this.lastResult = undefined;
				this.currentMethod = undefined;
			};

			// Store 'this' constructor (it is simply the function 'Core' itself)
//			var hack = this.constructor;

			// Prepare the cloning !
			Clone.prototype = this;
			Clone.prototype.contructor = Clone;

			// In fact, even before the next assignment, 'this.constructor' is still strictly equal to 'hack' value.
			// But in Chrome console, after the previous assignment, 'this' does not appear to be an instance of 'this.constructor' anymore !
			// Just by assigning again 'this.constructor' to its current value, solve the issue...
			// In conclusion:
			// Removing the variable 'hack' and the next assignment will have zero effect on the program behavior.
			// This hack is intended only for debugging purposes in the Chrome console.

//			this.constructor = hack; // Reasign 'this' constructor
			this.constructor = this.constructor.prototype.constructor;

			// Process the cloning !
			var clone = new Clone();

			// Clone callback and listeners
			...
			if (undefined === listeners || !!listeners) {
				// FIXME: faut-t-il faire une deep copy des propriété pour dupliquer entièrement les listeners ?
				// Actuellement, this._stack.listeners[i] est shared entre l'instance et tous ses clones...
				// En fait c'est pareille pour les fonctions enregitrées en callback...
				// Mais est-ce que ça pose un problème ?
...
console.log(d.event.core === core, d.event.core instanceof core.contructor);
